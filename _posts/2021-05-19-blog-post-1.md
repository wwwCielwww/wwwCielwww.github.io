...
- Shortest path between $v_s$ and $v_t$：$p_{st}=\arg \min_{p\in\mathcal{P}_{st}}\vert p\vert$, where $\mathcal{P}_{st}$ denotes the set of all paths between the two vertices.
- Diameter: $\text{diameter}(\mathcal{G})=\max _{v_{s}, v_{t} \in \mathcal{V}} \min _{p \in \mathcal{P}_{s t}}\vert p\vert$
