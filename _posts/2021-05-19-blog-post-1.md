...
An agent need to learn the $Q$  function through training, so that an action that would result in the maximum reward can be deduced at each state.
...
One may apply 2 different kinds of RL agents to a environment, namely the policy-based and value-based. The former directly gives the best action at a given state, whereas the later assigns a value to each state and derive an action from the value differences. 
