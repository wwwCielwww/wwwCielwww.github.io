...
To put things mathematically, denote node features of node $i$ in layer $(k-1)$ by $\mathbf{x}_i\in \mathbb{R}^F$ and (optional) edge features from node $j$ to node $i$ by $\mathbf{e}_{j, i}\in\mathbb{R}^D$. Message passing GNNs can be described as
